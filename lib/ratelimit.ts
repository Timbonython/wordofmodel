 ...
 * refused.
 */
export async function checkRateLimit(ipHash: string, kind: 'scan' | 'reveal' | 'waitlist' | 'login' | 'wizard' | 'review' | 'question'): Promise<RateVerdict> {
  const applicable =
    kind === 'scan'
      ? LIMITS
...
export async function recordAttempt(ipHash: string, kind: 'scan' | 'reveal' | 'waitlist' | 'login' | 'wizard' | 'review' | 'question'): Promise<void> {
